...
<main>
	<form id="register-form" class="register-login" autocomplete="on" data-actionurl="<?php echo $application->buildLink('register', 'register'); ?>" onsubmit="return false;">
		...
		<input type="password" placeholder="<?php echo __('Choose a Password'); ?>" name="password" title="<?php echo __('Password must be at least %s characters long.', $application->config['min_password_length']); ?>" autocomplete="off" minlength="<?php echo $application->config['min_password_length']; ?>" required>
		...
		<span id="validatorfor-password"><?php echo __('Passwords do not match.'); ?></span>
